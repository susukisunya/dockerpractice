...
1. コンテナイメージからコンテナを作成したいとき()
    - docker run -v ../現在いるフォルダの名前(dockerfile_c もしくは dockerfile_python):/volume --name コンテナの名前 コンテナイメージのタグ
1. コンテナを停止したいとき
    - docker stop コンテナの名前
...
1. コマンド一覧の2のコマンドを実行してコンテナを作成する
1. input.txtとoutput.txtに書いてある文字が同じであればAC、異なればWAと出力される
1. もし、input.txtに何も書かれていなければREと出力される
1. input.txt または output.txt の内容を変更してもう一度実行したいときは、コマンド一覧の2のコマンドをコンテナの名前を変えて実行してください
